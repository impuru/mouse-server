...
- **IP address changed** — your laptop's local IP can change between
  ...
  edit it in the phone app# Remote Mouse — control your laptop's mouse from your phone
...
- **Disconnects after ~1-2 minutes** — this was a bug in earlier versions
  where the server's event loop wasn't running in true cooperative mode,
  so it silently missed the WebSocket heartbeat. Fixed in this version; make
  sure you're running the latest `server.py` (and `pip install -r
  requirements.txt` again so `eventlet` is present). The page also now
  requests a screen wake lock and reconnects automatically when your
  phone's screen wakes, since screen lock was the other common cause.
